...
class TrainerSteplR(Trainer):
    ...
    def __get_lr_scheduler(self, optimizer):
        if self.configs.lr_scheduler=="step":
            ...
            return super().__get_lr_scheduler(optimizer)

    def event_before_training(self, network):
        if not self.__train_from_checkpoint:
            with open(self.project_path+"configs_network.yaml","w") as f:
                yaml.dump(network.configs_dict,f)

class DiffusionTrainner(TrainerSteplR):
    ...
    def set_configs_type(self):
        ...
        self.default_configs["diffusion_step"]=200
    
    def event_before_training(network,self):
    ...
    def train_step(self, network: torch.nn.Module, batched_data, idx_batch: int, num_batches: int, idx_epoch: int, num_epoch: int):
        condition = batched_data[0].to(device=self.device)
        targets = batched_data[1].to(device=self.device)
        t = torch.randint(1, self.diffuser.steps+1,
                          size=(targets.shape[0],), device=self.device)
        noise = torch.randn_like(targets, device=self.device)
        xt = self.diffuser.foward_diffusion(targets, t, noise)
...
class BNNTrainer(TrainerSteplR):
    ...
    def set_configs_type(self):
        super().set_configs_type()
        self.default_configs["KL_scaling"]=1
    
    def train_step(self, network: torch.nn.Module, batched_data, idx_batch: int, num_batches: int, idx_epoch: int, num_epoch: int):
        inputs = batched_data[0].to(device=self.device)
        targets = batched_data[1].to(device=self.device)
        prediction = network(inputs, torch.ones(size=(targets.shape[0],), device=self.device)*200, None)
        mseloss=torch.nn.functional.mse_loss(prediction, targets)
        klloss=get_kl_loss(network)*((2**(num_batches-(idx_batch+1)))/(2**num_batches-1))*self.KLscaling
